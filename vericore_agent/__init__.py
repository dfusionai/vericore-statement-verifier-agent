"""
Core Framework: vericore_agent/__init__.py

This module is the core framework that powers the Vericore Statement Verifier Agent system. 
It provides the infrastructure for running agents in isolated Docker containers 
and enabling RPC communication between validators and agents.

What It Provides:
1. RPC Server (FastAPI) - HTTP server for agent communication
2. Agent Lifecycle Management - Handles agent initialization and shutdown
3. Tool Registration & Execution - Dynamically registers and calls tools
4. Docker Container Orchestration - Manages isolated agent containers
5. Context Management - Tracks request metadata and state
...
logger = logging.getLogger("vericore_agent")
if not logger.handlers:
    _h = logging.StreamHandler(sys.stderr)
    _h.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] [vericore_agent] %(message)s"))
    logger.addHandler(_h)
logger.setLevel(getattr(logging, (os.getenv("VERICORE_LOGLEVEL") or "INFO").upper(), logging.INFO))
def _lvl(x):
...
def call_host(path: str, payload: dict, *, base_url: Optional[str] = None, timeout: int = 60):
    ...
    headers = {"x-vericore-token": os.getenv("VERICORE_TOKEN", "")}
...
# Entrypoints: decorator and compatibility alias for vericore_agent.boot
ENTRYPOINTS: dict[str, Callable[..., Any]] = {}
sys.modules.setdefault("vericore_agent.boot", sys.modules[__name__])
...
def entrypoint(_fn: Callable | None = None, *, name: str | None = None):
    def _decorator(fn: Callable) -> Callable:
        ...
        try:
            setattr(fn, "__vericore_entrypoint__", True)
            setattr(fn, "__vericore_entry_name__", name or fn.__name__)
        except Exception:
            pass
...
def _agent_entry_methods(instance: Any):
    try:
        for attr_name, attr in getattr(instance.__class__, "__dict__", {}).items():
            if callable(attr) and getattr(attr, "__vericore_entrypoint__", False):
                ep_name = getattr(attr, "__vericore_entry_name__", attr_name)
...
def load_env(env_or_path: str) -> Namespace:
    """
    Loads an environment configuration from either a local path or an installed package.
    
    The environment defines the Docker image, entrypoint method, and tool defaults
    needed to run agents. It expects a tools.py file that may contain:
    - DOCKER_IMAGE: The Docker image to use for containers (default: "python:3.11-slim")
    - ENTRYPOINT: The entrypoint method name (default: "solve")
    - TOOL_DEFAULTS: Optional default configuration for tools
    - register_tools(): Optional function to register tools at load time
    """
    p = Path(env_or_path)
    if p.exists():
        # Local file/directory mode: Load tools.py from the specified path
        # If path is a file, use its parent directory; if it's a dir, use it directly
        ...
        # Dynamically load the tools.py module from the file system
        tools_module = _load_module_from_file(f"vericore_env_{uuid.uuid4().hex}_tools", tools_file)
    else:
        # Installed package mode: Import from environments.{env_name}.tools module
    ...
    # Extract configuration from the tools module, using defaults if not specified
    ...
    # Return configuration as a Namespace object for easy attribute access
    return Namespace(
...
_CURRENT_CTX: ContextVar["Context | None"] = ContextVar("vericore_current_ctx", default=None)
...
def _validate_token(req: Request) -> str:
    tok = req.headers.get("x-vericore-token")
    ...
    try:
        return any(
            (p.name in {"ctx", "context"}) or (
                p.annotation in (Context, "Context", "vericore_agent.Context")
            )
...
def _instantiate_agents_and_register() -> dict[str, Any]:
    with _INIT_LOCK:
        ...
        ctx = Context(
            token=os.getenv("VERICORE_TOKEN", ""),
            method="startup",
            request_id=uuid.uuid4().hex,
            created_at=time.time(),
            headers={},
...
def ensure_server_running(*, host: str = "0.0.0.0", port: int = 5005, startup_timeout_s: float = 10.0) -> None:
    ...
    while time.time() < deadline:
        if _healthz_local_ok(port=port):
            return
        time.sleep(0.1)
    raise RuntimeError("Failed to start local Vericore server")
    ...
    try:
        for candidate in ["/app/tools.py", "/app/agent.py"]:
            if os.path.exists(candidate):
                _load_module_from_file(f"vericore_autoload_{Path(candidate).stem}", Path(candidate))
                logger.info(f"vericore_agent: loaded file {candidate}")
    except Exception as e:
        logger.warning(f"autoload failed: {e}")

def _bootstrap_container_token() -> None:
    fixed = os.getenv("VERICORE_TOKEN")
    if fixed and fixed not in _TOKEN_META:
        ...
        logger.info("vericore_agent: accepted container VERICORE_TOKEN for RPC")
...
async def _on_startup():
    ...
    try:
        ...
        logger.info(
            "vericore_agent: startup complete; registered %d methods; init=%s",
            len(_METHODS),
            res,
        )
    except Exception as e:
        logger.error("vericore_agent: initialization failed: %s", e)
        raise
...
class Container:
    def __init__(
        ...
        *,
        ...
        self.image = image or "thebes1618/vericore:latest"
        ...
        self.container_name = f"vericore-{os.path.splitext(os.path.basename(self.local_script_path))[0]}-{int(time.time())}-{uuid.uuid4().hex[:8]}"
        ...
        if self.base_url:
            ...
            if self.local_script_path and os.path.exists(self.local_script_path):
                ...
                # Copy our package into /app so `import vericore_agent` works without pip install
                ...
                exec_in_container_detach(
                    self.container_id,
                    "cd /app && /opt/venv/bin/python -m uvicorn vericore_agent:app --host 0.0.0.0 --port 5005",
                )
            else:
                # Image-only mode: assume image starts uvicorn vericore_agent:app
                self.container_id = start_container(
                    self.image,
                    self.container_name,
                    env={"VERICORE_TOKEN": self.token},
            ...
            logger.info(f"vericore_agent: container {self.container_id[:12]} listening at {self.base_url}")
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    logger.warning(f"vericore_agent: /methods before fail -> {getattr(r,'text',None)}")
                except Exception as e:
                    logger.warning(f"vericore_agent: /methods request failed: {e}")
    ...
    def _call(self, entry: str, *args, **kwargs):
        self._ensure_active()
        prev = os.environ.get("VERICORE_TOKEN")
        os.environ["VERICORE_TOKEN"] = self.token
        try:
            ...
            if prev is None:
                os.environ.pop("VERICORE_TOKEN", None)
            else:
                os.environ["VERICORE_TOKEN"] = prev
